...
**Статус документа**
//TODO переформировать таблицу

Номер бита | Пояснение
--- | ---
0..3 | Тип текущего открытого документа. 0 - документ закрыт, 1 , сервисный документ, 2 - чек на продажу (приход), 3 - чек на возврат (возврат прихода), 4 - внесение в кассу, 5 - инкассация, 6 - чек на покупку (расход), 7 - чек на возврат покупки (возврат расхода)
4..7 | Состояние документа. 0 - документ закрыт, 1 - устанавливается после команды "открыть документ" (Для типов документа 2 и 3 можно добавлять товарные позиции), 2 - Устанавливается после первой команды "Подытог", 3 - Устанавливается после второй команды "Подытог" или после начала команды «Оплата» (Можно только производить оплату различными типами платежных средств), 4 - Расчет завершен, требуется закрыть документ, 8 - Команда закрытия документа была дана в ФН, но документ не был завершен. Аннулирование документа невозможно  
...
### Ответные параметры
 * (Целое число) Статус печатающего устройтва
...
29 | Попытка добавить больше одного предмета расчета с тэгом 1214 имеющим значение 7
...
## Запрос дополнительной информации об ошибках (0x10)
### Описание
...
//TODO добавить таблицу системы налогооблажения
...
Доступен только один тип скидки: скидка на сумму (тип скидки = 1), при этом размер скидки не может превышаеть 99 копеек
...
Тип операции указывает, для какого типа операций регистрируется сумма по товарной группе:	0 – продажи, 	1 – возвратыю
...
//TODO таблица системы налогооблажения
### Ответные параметры
Нет
...
1 | 0 - приход, 1 - расход
2..4 | Система налогооблажения (см. ниже)
...
Система налогооблажения. Представляет собой битовую маску.
...
### Описание
Команда распечатывает документ из ФН. Если номер документа 0, то печатается список всех регистрацй/перерегистраций. Если установлен младший бит флагов, то документ не отрезается.
...
14 | Запрос версии ФН и версии ФФД | Строка, Число, Число, Строка, Число, Число, Число | Версия ФН (0 - отладочный ФН, 1 - серийный ФН) Версия ФФД (1209), Версия ККТ (1188), Версия ФФД ККТ (1189), Зарегистрированная версия ФФД ФН, Максимальная версия ФФД ФН (1190)
...
0 | 1 | 1 | 1 | 7 - Постфискальный режим. Идет передача ФД в ОФД
...
Значение параметра | Знаение скорости
--- | --- 
0 | 4800
1 | 9600
2 | 19200
...
5 | 0 - не печатать QR на чеке, 1 - печатат QR код на чеке //TODO можем ли печатать?
...
6 | 0 - нНе печатать дату и время начала смены, 1 - Печатать дату и время начала смены
...
### 10 - Логический номер ККТ (Число[1.9999])
Логический номер кассы, присвоенный ей системой данного торгового предприятия.

### 11 - Дополнительная ячейка (Число[1.65535])
Ячейка таблицы, которую внешнее ПО может использовать для своих целей.

### 12 - Настройки ПУ (Массив 1..3)
 * 1 элемент - коррекция яркости печати (20..100%)
 * 2 элемент - коррекция частичной отрезки для бумаги 80 мм (0…100%)
 * 3 элемент - коррекция частичной отрезки для бумаги 57 мм (0…100%)

### 20 - Пароль для связи (Строка[4])
Пароль на связь, 4 символа

### 30 - Наименование и адрес организации (Массив 0..3)
 * 0 элемент - (Строка[0..44]) Наименование организации, 1-ая строка  
 * 1 элемент - (Строка[0..44]) Наименование организации, 2-ая строка
 * 2 элемент - (Строка[0..44]) Адрес организации, 1-ая строка
 * 3 элемент - (Строка[0..44]) Адрес организации, 2-ая строка
1 и 3 элемент может быть пустым. В этом случае эти строки не печатаются

### 31 - Строки окончания чеков (Массив 0..4)
Все элементы - (Строка[0..44]) Строки, печатаемые в конце каждого чека. Пустые строки не печатаются.

### 32 - Названия типов платежей (Массив 0..15)
0 элемент - (Строка[0..18]) Зарезервирован типом “НАЛИЧНЫМИ” (только чтение)
1 элемент - (Строка[0..18]) Зарезервирован типом “ЭЛЕКТРОННЫМИ” (только чтение)
2..12 элементы (Строка[0..18]) Строка наименования платежа. Если строка пустая – платеж не может быть использован для расчетов (TODO или может?)
13 элемент - (Строка[0..18]) Зарезервирован типом "ПРЕДВАРИТЕЛЬНАЯ ОПЛАТА (АВАНС)" (только чтение)
14 элемент - (Строка[0..18]) Зарезервирован типом "ПОСЛЕДУЮЩАЯ ОПЛАТА (КРЕДИТ)" (только чтение)
15 элемент - (Строка[0..18]) Зарезервирован типом "ИНАЯ ФОРМА ОПЛАТЫ" (только чтение)